...
`tmux-superchat` is **read-only**. It makes two kinds of HTTPS request to the configured
`@superchat-api` origin:
...
`~/.config/tmux-superchat`, and stores no personal data beyond a random install id.
